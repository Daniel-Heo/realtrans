...
class CTrans:
    ...
    def translate(self, source, source_lang, target_lang, lang_map=None):
        ...
        # source = source.replace('…','.')
        # source = source.replace('¿','?')
        # source를 for문으로 char단위로 순회하면서 .이면 다음 char가 숫자가 아니면 다음 문장으로.
        input_list = []
        start_pos = 0
        ...
        if True:
            default_trans_options =  {
                "max_batch_size": 256, # 최대 배치 크기입니다. 입력 수보다 크면 max_batch_size입력은 길이별로 정렬되고 max_batch_size예제 청크로 분할되어 패딩 위치 수가 최소화됩니다.
            ...
            results = self.translator.translate_batch(source_sentences, target_prefix=target_prefix, **default_trans_options)
            translation = ""
